...
export default function ServiceBookingBottomSheet({
  ...
  // Salon hours logic
  const isTuesday = selectedDate ? new Date(selectedDate).getDay() === 2 : false;
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'open': return colors.status?.success || '#10B981';
      case 'closed': return colors.status?.error || '#EF4444';
      case 'bookings-disabled': return colors.status?.warning || '#F59E0B';
      default: return colors.status?.info || '#3B82F6';
    }
  };

  const getStatusMessage = (status) => {
    switch (status) {
      case 'open': return 'Salon is open for business';
      case 'closed': return isTuesday ? 'Tuesday is our weekly closure day' : 'Salon is closed today';
      case 'bookings-disabled': return 'Salon is open but bookings are disabled';
  ...
  const openTimePicker = (type) => {
    const currentTime = type === 'open' ? salonHoursState.openTime : salonHoursState.closeTime;
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
        ...
        if (event.absoluteY < 100) { // Only handle gestures from top 100px
                    ...
                    >
                      ...
                      <View style={styles.section}>
                        <View style={styles.sectionHeader}>
                          ...
                          <ThemedText style={styles.sectionTitle}>SALON STATUS</ThemedText>
                        </View>
                        <View style={[styles.statusCard, { backgroundColor: getStatusColor(getSalonStatus()) }]}>
                          <View style={styles.statusRow}>
                            <Ionicons 
                              name={getSalonStatus() === 'open' ? 'checkmark-circle' : 'close-circle'} 
                              size={24} 
                              color="white" 
                            />
                            <ThemedText style={styles.statusText}>
                              {getSalonStatus() === 'open' ? 'Salon Open' : 
                               getSalonStatus() === 'closed' ? 'Salon Closed' : 'Bookings Disabled'}
                            </ThemedText>
                            {isTuesday && salonHoursState.isClosed && (
                              <View style={styles.tuesdayBadge}>
                                <ThemedText style={styles.tuesdayBadgeText}>TUESDAY</ThemedText>
                              </View>
                            )}
                          </View>
                      ...
                      <View style={styles.section}>
                        ...
                        <View style={styles.hoursCard}>
                          <View style={styles.timeRow}>
                            <ThemedText style={styles.timeLabel}>Open Time:</ThemedText>
                            <TouchableOpacity 
                              style={[styles.timeButton, salonHoursState.isClosed && styles.timeButtonDisabled]}
                              onPress={() => !salonHoursState.isClosed && openTimePicker('open')}
                              disabled={salonHoursState.isClosed}
                            >
                              <ThemedText style={[styles.timeText, salonHoursState.isClosed && styles.timeTextDisabled]}>
                          ...
                          <View style={styles.timeRow}>
                            <ThemedText style={styles.timeLabel}>Close Time:</ThemedText>
                            <TouchableOpacity 
                              style={[styles.timeButton, salonHoursState.isClosed && styles.timeButtonDisabled]}
                              onPress={() => !salonHoursState.isClosed && openTimePicker('close')}
                              disabled={salonHoursState.isClosed}
                            >
                              <ThemedText style={[styles.timeText, salonHoursState.isClosed && styles.timeTextDisabled]}>
                      ...
                      <View style={styles.section}>
                        <View style={styles.sectionHeader}>
                          ...
                          <ThemedText style={styles.sectionTitle}>DAY SETTINGS</ThemedText>
                        </View>
                        <View style={styles.settingsCard}>
                          <TouchableOpacity 
                            style={styles.settingRow}
                            onPress={() => handleSalonHoursChange('isClosed', !salonHoursState.isClosed)}
                          >
                            <Ionicons 
                              name={salonHoursState.isClosed ? 'checkbox' : 'square-outline'} 
                              size={24} 
                              color="white" 
                            />
                            <ThemedText style={styles.settingText}>Salon Closed Today</ThemedText>
                          </TouchableOpacity>
                          
                          <TouchableOpacity 
                            style={styles.settingRow}
                            onPress={() => handleSalonHoursChange('disableBookings', !salonHoursState.disableBookings)}
                          >
                            <Ionicons 
                              name={salonHoursState.disableBookings ? 'checkbox' : 'square-outline'} 
                              size={24} 
                              color="white" 
                            />
                            <ThemedText style={styles.settingText}>Disable Bookings Only</ThemedText>
                          </TouchableOpacity>
                          
                          <TouchableOpacity 
                            style={styles.settingRow}
                            onPress={() => handleSalonHoursChange('isHoliday', !salonHoursState.isHoliday)}
                          >
                            <Ionicons 
                              name={salonHoursState.isHoliday ? 'checkbox' : 'square-outline'} 
                              size={24} 
                              color="white" 
                            />
                            <ThemedText style={styles.settingText}>Mark as Holiday</ThemedText>
                          </TouchableOpacity>
                        </View>
                      </View>
                      ...
                      <View style={styles.section}>
                        ...
                        <View style={styles.notesCard}>
                          <TextInput
                            ...
                            onChangeText={(text) => handleSalonHoursChange('notes', text)}
                            multiline
                            maxLength={200}
                          />
          ...
          <View style={styles.timePickerModal}>
            <View style={styles.timePickerContainer}>
              <View style={styles.timePickerHeader}>
                ...
                <TouchableOpacity onPress={cancelTimeChange} style={styles.timePickerCloseButton}>
              ...
              <View style={styles.timePickerActions}>
                ...
                >
                  <ThemedText style={styles.timePickerCancelText}>Cancel</ThemedText>
                </TouchableOpacity>
                <TouchableOpacity 
                  style={[
                    styles.timePickerConfirmButton,
                    !isTimeSelectionValid() && styles.timePickerConfirmButtonDisabled
                  ]}
                ...
                >
                  <ThemedText style={[
                    ...
                    !isTimeSelectionValid() && styles.timePickerConfirmTextDisabled
                  ]}>
                    Confirm
                  </ThemedText>
                </TouchableOpacity>
